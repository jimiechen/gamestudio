...
```
  ...
  其他负责人（向 producer/directors 报告）：
    release-manager         -- 发布流程、版本控制、部署
    localization-lead       -- 国际化、字符串表、翻译流程
  ...
  引擎专家（使用与你的引擎匹配的套件）：
    unreal-specialist  -- UE5 负责人：蓝图/C++、GAS 概述、UE 子系统
      ue-gas-specialist         -- GAS：能力、效果、属性、标签、预测
      ue-blueprint-specialist   -- 蓝图：BP/C++ 边界、图表标准、优化
    ...
    unity-specialist   -- Unity 负责人：MonoBehaviour/DOTS、Addressables、URP/HDRP
      ...
      unity-addressables-specialist -- 资源：异步加载、包、内存、CDN
...
| [engine] 子专家 | （向所有程序员建议引擎子系统模式和优化） |
| live-ops-designer | economy-designer（实时经济）、community-manager（活动沟通）、analytics-engineer（参与度指标） |
| community-manager | （与 producer 合作获得批准，与 release-manager 合作确定补丁说明时间） |
...
| 部门间日程冲突 | producer |
| 范围超出能力 | producer，然后 creative-director 进行削减 |
...
```
1. analytics-engineer -- 从数据中识别不平衡（或玩家报告）
2. game-designer      -- 根据设计意图评估问题
3. economy-designer   -- 建模调整
4. game-designer      -- 批准新值
5. [data file update] -- 更改配置值
6. qa-tester          -- 对受影响系统进行回归测试
7. analytics-engineer -- 监控更改后的指标
```

### 模式 4：新区域/关卡

```
1. narrative-director -- 定义区域的叙事目的和节奏
...
6. devops-engineer      -- 构建发布产物，运行部署流程
7. release-manager      -- 生成 /changelog，标记发布，创建发布说明
8. technical-director   -- 对重大发布进行最终签署
...
5. prototyper           -- 记录发现报告
6. creative-director    -- 对进入生产阶段进行 go/no-go 决策
7. producer             -- 如获批准，安排生产工作
```
...
当美术圣经或资源标准更改时，art-director 必须通知：
- technical-artist（流程更改）
- 所有使用受影响资源的内容创作者
- devops-engineer（如果构建流程受影响）

## 要避免的反模式

1. **绕过层级**：专业智能体绝不应在未经协商的情况下做出属于其负责人的决策。
2. **跨域实现**：智能体绝不应未经相关所有者明确委派就修改其指定区域之外的文件。
3. **影子决策**：所有决策都必须记录。没有书面记录的口头协议会导致矛盾。
4. **整体任务**：分配给智能体的每个任务应该在 1-3 天内可完成。如果更大，必须先分解。
5. **基于假设的实现**：如果规范不明确，实现者必须询问规范者而不是猜测。错误的猜测比提问更昂贵。
